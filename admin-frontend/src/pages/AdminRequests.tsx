...
import ChangeStatusDialog from "@/components/admin/requests/ChangeStatusDialog";
import DeleteRequestDialog from "@/components/admin/requests/DeleteRequestDialog";
import UserProfileDialog from "@/components/admin/requests/UserProfileDialog";
import TechnicianProfileDialog from "@/components/admin/technicians/TechnicianProfileDialog";
import { toast } from "@/hooks/use-toast";
import AssignTechnicianDialog from "@/components/admin/requests/AssignTechnicianDialog";
...
const AdminRequests = () => {
      ...
      try {
        ...
        const formatted = data.map((t: any) => ({
          ...
          files: (t.attachments || []).map((a: any) => ({
            name: a.name,
            url: `https://localhost:7181/${a.url}`,
  ...
  const handleChangeStatus = async (
    ...
    try {
      ...
      // 2. Send the status update email
      await fetch(
        `https://localhost:7181/api/notification/send-status-update/${requestId}`,
        {
          method: "POST",
        }
      );
  ...
  const handleAssignTechnician = async (
    ...
    try {
      // 1. Assign the technician in backend
      await fetch(
      ...
      // 2. Send status update email (same endpoint)
      await fetch(
        `https://localhost:7181/api/notification/send-status-update/${requestId}`,
        {
          method: "POST",
                      ...
                      >
                        ...
                        <TableCell>
                          <Button
                            ...
                            onClick={() =>
                              handleViewUserProfile(request.userId)
                            }
                          >
                            {request.userName}
                          </Button>
